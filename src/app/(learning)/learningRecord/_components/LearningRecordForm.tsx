...
import {
  ...
  LearningRecordSchema,
} from "../_utils/learningRecordSchema.ts"; //ts消すとエラー
import {
...
const LearningRecordForm: React.FC<LearningRecordFormProps> = ({
  categories,
}) => {
  ...
  const onSubmit: SubmitHandler<LearningRecordSchema> = async (data) => {
    try {
      // PrismaのDateTime型に合わせて、日付と時間を結合したDateオブジェクトを作成
      const startTime = new Date(data.learningDate);
      startTime.setHours(
        parseInt(data.startHour, 10),
        parseInt(data.startMinute, 10),
        0,
        0
      );

      const endTime = new Date(data.learningDate);
      endTime.setHours(
        parseInt(data.endHour, 10),
        parseInt(data.endMinute, 10),
        0,
        0
      );

      const learningDate = new Date(data.learningDate);
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="rounded-lg border text-card-foreground bg-white/80 backdrop-blur-sm border-green-100 shadow-sm">
            ...
            <div className="p-6 pt-0 space-y-4">
              ...
              <div>
                <label htmlFor="learningDate">学習日</label>
                <input
                  id="learningDate"
                  type="date"
